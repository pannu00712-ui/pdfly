...
module.exports = async function handler(req, res) {
  ...
  try {
    const summary = await chatCompletion([
      { role: 'system', content: 'You summarize documents concisely and accurately in clear paragraphs. Do not invent facts not present in the source text.' },
      { role: 'user', content: `Summarize the following document:\n\n${text}` },
    ], { maxTokens: 700 });

    return res.status(200).json({ summary });
  } catch (e) {
    console.error('[api/pdf-summarizer]', e.message);
